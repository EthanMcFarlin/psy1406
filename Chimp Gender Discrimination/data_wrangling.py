...
for filename in os.listdir(face_images_dir):
    if filename.endswith('.png'):
        ...
        if gender and gender_counters[gender] < 50:
            new_name = filename
            ...
            if all(count >= 50 for count in gender_counters.values()):
                break
        else:
            # Skip the file if the gender count has been reached or if no gender is found
            continue

# Step 6: Print the names of all files in the renamed folder in the specified format
anchor_image = 'img-id891-object-1_Anchor_Male.png'
image_pairs = []
renamed_files = os.listdir(renamed_images_dir)
for filename in renamed_files:
    if filename != anchor_image:  # Exclude the anchor image from the right side of the pairs
        image_pairs.append((anchor_image, filename))

# Print the image pairs in the specified format
print("image_pairs = [")
